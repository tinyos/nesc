...
declares multiple interfaces to this component, one for each distinct list
...
$F'$ for $I.F$, then $F'$ is called with parameter values $P$.
\item If $S_1 \cup S_2$ is non-empty: the elements of $S_1 \cup S_2$ are
called in an arbitrary order. The elements of $S_1$ receive $P$ as their
parameter values. The parameterised elements $(X, L_P)$ of $S_2$ receive
the first element of $L_P$ as parameter values.
\end{itemize}

The result of $M.I.F$ is the result of one of the functions called (note that
